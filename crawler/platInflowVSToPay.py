...
def handleData(returnStr):
    ...
    dataList2=jsonData.get('data2').get('y1')
    dataList3=jsonData.get('data2').get('y2')
    return dateList,dataList1,dataList2,dataList3
def storeData(dateStr,dataStr1,dataStr2,dataStr3,conn,cur,wdzjPlatId):
    sql='insert into platInflowVSToPay (date,inflow,toPay,toPay30day,wdzjPlatId) values ("'+dateStr+'","'+dataStr1+'","'+dataStr2+'","'+dataStr3+'","'+wdzjPlatId+'")'
    cur.execute(sql)
    conn.commit()
...
for i in mylist:
    ...
    try:
        ...
        dateList,dataList1,dataList2,dataList3=handleData(data)
        for j in range(len(dateList)):
            dateStr=str(dateList[j])
            dataStr1=str(dataList1[j])
            dataStr2=str(dataList2[j])
            dataStr3=str(dataList3[j])
            storeData(dateStr,dataStr1,dataStr2,dataStr3,conn,cur,str(i))
    except Exception,e:
        traceback.print_exc()
cur.close()
conn.close