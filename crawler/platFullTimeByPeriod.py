...
def handleData(returnStr):
    ...
    dataList1=jsonData.get('data1').get('y6')
    dataList2=jsonData.get('data1').get('y7')
    dataList3=jsonData.get('data1').get('y8')
    dataList4=jsonData.get('data1').get('y9')
    dataList5=jsonData.get('data1').get('y10')
...
def storeData(dateStr,dataStr1,dataStr2,dataStr3,dataStr4,dataStr5,conn,cur,wdzjPlatId):
    sql='insert into platFullTimeByPeriod (date,day,oneMonth,twoMonth,threeMonth,sixMonth,wdzjPlatId) values ("'+dateStr+'","'+dataStr1+'","'+dataStr2+'","'+dataStr3+'","'+dataStr4+'","'+dataStr5+'","'+wdzjPlatId+'")'
    cur.execute(sql)
    conn.commit()
