...
def storeData(dateStr,dataStr1,dataStr2,dataStr3,dataStr4,conn,cur,wdzjPlatId):
    sql='insert into platInvestorGrade (date,qianPerson,wanPerson,shiWanPerson,baiWanPerson,wdzjPlatId) values ("'+dateStr+'","'+dataStr1+'","'+dataStr2+'","'+dataStr3+'","'+dataStr4+'","'+wdzjPlatId+'")'
    cur.execute(sql)
    conn.commit()
